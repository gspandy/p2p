...
public class CrmAction extends BaseAction {
	...
	public String feedbackadd() {
		...
		Account account = accountDao.selectById(accountid);
		request.setAttribute("account", account);
		return layout("crm", "feedback", "客户来电", "/crm/feedbackadd.ftl", SystemConstant.LAYOUT_ADMIN);
	}
	...
	public String feedbacksave() {
		...
		feedback.setManager(manager.getRealname());
		feedback.setChannel(Feedback.CHANNEL_CALL);
		feedback.setReplytime(new Date());
		feedback.setStatus(Feedback.STATUS_REPLIED);
		feedbackDao.insert(feedback);
		return redirect("/admincrm/feedback.html?accountid=" + feedback.getAccountid());
	}
